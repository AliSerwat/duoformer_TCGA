# python step_4_ResNet50_Utah_20240419_4_classes_20X_UtahSwavExp126_2.py
...
os.environ['CUDA_VISIBLE_DEVICES']='2'#T2 may need to put it before import torch
...
def train(optimizer, epoch, train_only_labeled_loader, num_iter_in_one_epoch):
    ...
    f = open(save_results_folder+'/20240419_UtahSwavExp126_2_train01_{}.txt'.format(my_modelname), 'a+')
...
#######################This block is for visualizing results#########################################
            if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
                path_val_scores=save_results_folder+'/20240419_UtahSwavExp126_2_val_scores.txt'
...
#######################This block is for visualizing results#########################################
            if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
                path_test_scores=save_results_folder+'/20240419_UtahSwavExp126_2_test_scores.txt'
        ...
        if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
            path_val_scores=save_results_folder+'/20240419_UtahSwavExp126_2_test_scores.txt'
            f = open(path_val_scores, 'a+')
            f.write('confusion matrix: each row counts prediction, each column counts ground truth\n')
            for row in range(NUM_CLASSES):
                for col in range(NUM_CLASSES):
...
if __name__ == '__main__':
    ...
    if program_mode !='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
        ...
        if program_mode=='resume_best_training':# 'normal_training', 'resume_best_training', 'resume_latest_training', 'only_test'
            ...
            path2weights=save_models_folder+'/20240419_UtahSwavExp126_2_current_best.pt'
            my_model.load_state_dict(torch.load(path2weights))
            current_result_path=save_results_folder+'/20240419_UtahSwavExp126_2_current_best_for_resuming.txt'
            ...
            path2weights=save_models_folder+'/20240419_UtahSwavExp126_2_current_latest.pt'
            my_model.load_state_dict(torch.load(path2weights))
            current_result_path=save_results_folder+'/20240419_UtahSwavExp126_2_current_latest_for_resuming.txt'
        ...
        for epoch in range(start_epoch, total_epoch+1):
            ...
            f = open(f'{save_results_folder}/20240419_UtahSwavExp126_2_val01_{my_modelname}.txt', 'a+')
            ...
            if best_acc<balanced_val_acc:
                ...
                get_best_model=1
                torch.save(my_model.state_dict(), save_models_folder+"/20240419_UtahSwavExp126_2_current_best.pt".format(my_modelname))
                ...
                now = datetime.now()
                current_time = now.strftime("%H:%M:%S")
                f = open(f'{save_results_folder}/20240419_UtahSwavExp126_2_test01_{my_modelname}.txt', 'a+')
            ...
            if epoch%save_latest_epoch_frequency==0:
                torch.save(my_model.state_dict(), save_models_folder+"/20240419_UtahSwavExp126_2_current_latest.pt".format(my_modelname))
                current_result_path=save_results_folder+'/20240419_UtahSwavExp126_2_current_latest_for_resuming.txt'
                current_result_file = open(current_result_path, 'w')#overwrite
                current_result_file.write(f'{epoch}\n')
                current_result_file.write(f'{balanced_val_acc}\n')
    ...
    if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
        path2test_weights=save_models_folder+"/20240419_UtahSwavExp126_2_current_best.pt"
        ...
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        f = open(f'{save_results_folder}/20240419_UtahSwavExp126_2_val01_load_model_{my_modelname}.txt', 'a+')
        ...
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        f = open(f'{save_results_folder}/20240419_UtahSwavExp126_2_test01_load_model_{my_modelname}.txt', 'a+')
        ...
        f.close()
    if get_best_model==1:
        torch.save(my_best_model.state_dict(), save_models_folder+"/20240419_UtahSwavExp126_2_val01_{}_{}_balanced_val_acc{}_balanced_test_acc{}.pt".format(my_modelname,best_epoch,best_acc,best_epoch_balanced_test_acc))
