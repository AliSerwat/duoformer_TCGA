# python step_4_ResNet_20230328_Utah_4_classes_fixed_H_E_exp63_0.py
# ResNet
#Things to change:
...
import warnings
from models_three_stage_vit_swin import * # to use vision transformer variations

warnings.filterwarnings('ignore')
os.environ['CUDA_VISIBLE_DEVICES']='0'#P2 may need to put it before import torch
...
initiall_lr=1e-5 #0.0001
# lr_decay_factor=0.9
# lr_patience=5
weight_decay= 1e-4 # 1e-8,for optimizer
total_epoch = 50
...
NUM_CLASSES=4
CLASSES_TYPES=['Normal_Type', 'Low_Risk', 'High_Risk', 'Necrosis']
...
my_modelname = 'r18swintransformerB24_run3' # pretrained_my_model_hybrid_case1
bs=200#how frequent to show on terminal, show once each bs batches
...
save_models_folder= f'r18swintransformer/{my_modelname}_backup'
save_results_folder= f'r18swintransformer/{my_modelname}_result'
scales = 2
depth= 8  # number of blocks
proj_dim = 512 # proj_dim: 384
heads = 8 # 12
mlp_ratio = 4.
patch_size=32 # 224 // 7
num_patches = 49
...
backbone = 'r18' # 'r18,r50SwAV,r50
init_values = None
pretrained = True
scale_token = 'channel' # channel, random, first, avg
patch_attn=True
...
def train(optimizer, epoch, train_only_labeled_loader, num_iter_in_one_epoch):
    ...
    f = open(save_results_folder+'/20230328_exp63_0_train01_{}.txt'.format(my_modelname), 'a+')
...
#######################This block is for visualizing results#########################################
            if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
                path_val_scores=save_results_folder+'/20230328_exp63_0_val_scores.txt'
...
#######################This block is for visualizing results#########################################
            #if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
            path_test_scores=save_results_folder+'/20230328_exp63_0_test_scores.txt'
        ...
        #if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
        path_val_scores=save_results_folder+'/20230328_exp63_0_test_scores.txt'
        f = open(path_val_scores, 'a+')
        f.write('confusion matrix: each row counts prediction, each column counts ground truth\n')
        for row in range(NUM_CLASSES):
            for col in range(NUM_CLASSES):
...
if __name__ == '__main__':
    if WORK_ON_LAB_SERVER:
        txt_root_dir='./datasets/UTAH/202302_4_classes_tile_list/'
        tile_loc='./datasets/UTAH/dataset/'
    else:
        txt_root_dir=''#'/uufs/chpc.utah.edu/common/HIPAA/proj_knudsen/TCGA-H&E-Images/kirc_Bodong/dataset/dataset_txt_files_20230301_20X_train_labeled_unlabeled_val_test/'
    ...
    trainset_only_labeled = GPDataset(
                            ...
                            list_of_each_GP_txt_path=[
                                txt_root_dir+'20230308_train_labeled_0_Normal_Type_tiles_list.txt',
                                txt_root_dir+'20230308_train_labeled_1_Low_Risk_tiles_list.txt',
                                txt_root_dir+'20230308_train_labeled_2_High_Risk_tiles_list.txt',
                                txt_root_dir+'20230308_train_labeled_3_Necrosis_tiles_list.txt'],
    ...
    valset = GPDataset(
                            root_dir_RGB=tile_loc,
                            list_of_each_GP_txt_path=[
                                txt_root_dir+'20230308_val_0_Normal_Type_tiles_list.txt',
                                txt_root_dir+'20230308_val_1_Low_Risk_tiles_list.txt',
                                txt_root_dir+'20230308_val_2_High_Risk_tiles_list.txt',
                                txt_root_dir+'20230308_val_3_Necrosis_tiles_list.txt'],
                            dataset_type='validation')
    testset = GPDataset(
                            root_dir_RGB=tile_loc,
                            list_of_each_GP_txt_path=[
                                txt_root_dir+'20230308_test_0_Normal_Type_tiles_list.txt',
                                txt_root_dir+'20230308_test_1_Low_Risk_tiles_list.txt',
                                txt_root_dir+'20230308_test_2_High_Risk_tiles_list.txt',
                                txt_root_dir+'20230308_test_3_Necrosis_tiles_list.txt'],
                            dataset_type='test')
    ...
    '''r50 pretrained on IN(ImageNet)'''
    # my_model = models.resnet50(pretrained=True)#.cuda()#changable origin resnet18
    # num_ftrs = my_model.fc.in_features
    # my_model.fc = nn.Linear(num_ftrs, NUM_CLASSES)
    # # Freeze all the layers except the newly created fc layer
    # if freeze_backbone:
    ...
    '''r50 pretrained on pathological datasets(TCGA+TULIP)'''
    # my_model = ResNet50withFC(pretrained=True, progress=False, num_classes=NUM_CLASSES, key="BT", freeze=freeze_backbone) # MoCoV2, SwAV, BT 
    # # Check if the parameters of the feature extractor are frozen
    # for name, param in my_model.resnet_trunk.named_parameters(): 
    ...
    #     if all_frozen:
    #         print("Success: All feature extractor layers are frozen.")
    #     else:
    #         print("Error: Not all feature extractor layers are frozen.")
    #     print(count_parameters(my_model))


    '''Scaleformer'''
    # my_modelname = 'scaleformer' #changable#####################'ResNet18''R50ViT''R50ViTPretrained'
    # my_model = build_hybrid(num_classes=NUM_CLASSES,num_blocks=depth,proj_dim=proj_dim,num_heads=heads,backbone=backbone,freeze_backbone=freeze_backbone) # pretrained/unprertrained hybrid model
    # my_model = build_model(depth=depth,patch_size=patch_size, embed_dim=proj_dim,num_heads=heads, init_values = init_values,num_classes=NUM_CLASSES,num_layers = scales, 
    #             proj_dim = proj_dim,pretrained=pretrained,freeze=freeze_backbone)         # previous code of scaleformer
    ...
    #         freeze_backbone = freeze_backbone, backbone = backbone,pretrained=pretrained,scale_token = scale_token,
    #         patch_attn=patch_attn) # current code of scaleformer, SwAV used in model_wo_extra_params.
    # my_model = build_three_stage_vit(num_classes=NUM_CLASSES,num_blocks=depth,proj_dim=proj_dim,num_heads=heads,backbone=backbone,freeze_backbone=freeze_backbone) # three-stage-vit
    my_model = build_r50_swin_transformer(num_classes=NUM_CLASSES,backbone=backbone,freeze_backbone=freeze_backbone)  
    print('trainable vs total parameters:',count_parameters(my_model))
    ...
    if program_mode !='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
        ...
        if program_mode=='resume_best_training':# 'normal_training', 'resume_best_training', 'resume_latest_training', 'only_test'
            ...
            path2weights=save_models_folder+'/20230328_exp63_0_current_best.pt'
            my_model.load_state_dict(torch.load(path2weights))
            current_result_path=save_results_folder+'/20230328_exp63_0_current_best_for_resuming.txt'
            ...
            path2weights=save_models_folder+'/20230328_exp63_0_current_latest.pt'
            my_model.load_state_dict(torch.load(path2weights))
            current_result_path=save_results_folder+'/20230328_exp63_0_current_latest_for_resuming.txt'
        ...
        optimizer = optim.Adam(my_model.parameters(), lr=initiall_lr,weight_decay=weight_decay)
        # optimizer = optim.RMSprop(my_model.parameters(), lr=initiall_lr, weight_decay=weight_decay, momentum=0.9)
        ...
        scheduler = optim.lr_scheduler.OneCycleLR(optimizer, max_lr=initiall_lr, steps_per_epoch=MANUAL_SET_num_iter_in_one_epoch, epochs=total_epoch)
        ...
        for epoch in range(start_epoch, total_epoch+1):
            ...
            f = open(f'{save_results_folder}/20230328_exp63_0_val01_{my_modelname}.txt', 'a+')
            ...
            if best_acc<balanced_val_acc:
                ...
                get_best_model=1
                torch.save(my_model.state_dict(), save_models_folder+"/20230328_exp63_0_current_best.pt".format(my_modelname))
                test_target_list, test_score_list, test_pred_list, raw_unbalanced_test_acc, test_loss = test(my_model)
                ...
                now = datetime.now()
                current_time = now.strftime("%H:%M:%S")
                f = open(f'{save_results_folder}/20230328_exp63_0_test01_{my_modelname}.txt', 'a+')
            ...
            if epoch%save_latest_epoch_frequency==0:
                torch.save(my_model.state_dict(), save_models_folder+"/20230328_exp63_0_current_latest.pt".format(my_modelname))
                current_result_path=save_results_folder+'/20230328_exp63_0_current_latest_for_resuming.txt'
                current_result_file = open(current_result_path, 'w')#overwrite
                current_result_file.write(f'{epoch}\n')
                current_result_file.write(f'{balanced_val_acc}\n')
    ...
    if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
        path2test_weights=save_models_folder+"/20230328_exp63_0_current_best.pt"
        ...
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        f = open(f'{save_results_folder}/20230328_exp63_0_val01_load_model_{my_modelname}.txt', 'a+')
        ...
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        f = open(f'{save_results_folder}/20230328_exp63_0_test01_load_model_{my_modelname}.txt', 'a+')
        ...
        f.close()
    if get_best_model==1:
        torch.save(my_best_model.state_dict(), save_models_folder+"/20230328_exp63_0_val01_{}_{}_balanced_val_acc{}_balanced_test_acc{}.pt".format(my_modelname,best_epoch,best_acc,best_epoch_balanced_test_acc))
