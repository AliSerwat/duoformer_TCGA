...
class PatchBlock(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()    
        self.attn = AttentionForPatch(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=proj_drop)
...
class MultiscaleFormer(nn.Module):
    ...
    def __init__(self,
        ...
        self.scaleBlocks = nn.Sequential(*[
            ...
            for i in range(depth)])
        self.blocks = nn.Sequential(*[
            block_fn1(dim=embed_dim, num_heads=num_heads, qkv_bias=qkv_bias,proj_drop=proj_drop_rate,attn_drop=attn_drop_rate
            )
            for i in range(depth)])
