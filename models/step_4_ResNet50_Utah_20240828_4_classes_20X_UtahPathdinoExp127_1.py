# python step_4_ResNet50_Utah_20240828_4_classes_20X_UtahPathdinoExp127_1.py
# cd /uu/sci.utah.edu/tolga_scratch/bodong/projects/Xiaoya_Transformer/programs_and_results/models/
# 2024 0828: loss calculated by torch.mean instead of torch.sum. 
# exp 124: FREEZE_BACKBONE=True
...
from resnet50ssl import PathDinoClassifier, ResNet50withFC
import warnings
warnings.filterwarnings('ignore')
os.environ['CUDA_VISIBLE_DEVICES']='1'#C1 may need to put it before import torch
...
batchsize=64 #29100MB GPU for bs255
...
training_tile_size=256#changed for exp 127 pathdino512
my_modelname = 'pathdino512'#changed for exp 127 pathdino512
...
save_models_folder='model_ResNet50_backup'
save_results_folder='model_ResNet50_result'
...
train_transformer = transforms.Compose([
    ...
    transforms.Resize(512),#changed for exp 127 pathdino512
...
def train(optimizer, epoch, train_only_labeled_loader, num_iter_in_one_epoch):
    ...
    for iter_index in range(num_iter_in_one_epoch):
        ...
        loss_cross_entropy=torch.mean(loss_cross_entropy_batch)
        ...
        if batch_index % bs == 0:
            print('Train Epoch: {} [{}/{} ({:.4f}%)]\tTrain Loss: {:.6f}'.format(
                ...
                100.0 * (batch_index+1) / num_iter_in_one_epoch, loss_cross_entropy.item()))
        batch_index+=1
    ...
    f = open(save_results_folder+'/20240828_UtahPathdinoExp127_1_train01_{}.txt'.format(my_modelname), 'a+')
...
#######################This block is for visualizing results#########################################
            if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
                path_val_scores=save_results_folder+'/20240828_UtahPathdinoExp127_1_val_scores.txt'
                ...
                if batch_index%10==0:
                    print(f'working on batch {batch_index}\n')
                f.close()
                if batch_index==0:
...
#######################This block is for visualizing results#########################################
            if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
                path_test_scores=save_results_folder+'/20240828_UtahPathdinoExp127_1_test_scores.txt'
        ...
        if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
            path_val_scores=save_results_folder+'/20240828_UtahPathdinoExp127_1_test_scores.txt'
            f = open(path_val_scores, 'a+')
            f.write('confusion matrix: each row counts prediction, each column counts ground truth\n')
            for row in range(NUM_CLASSES):
                for col in range(NUM_CLASSES):
...
if __name__ == '__main__':
    if WORK_ON_LAB_SERVER:
        txt_root_dir='/uu/sci.utah.edu/tolga_scratch/public_dataset/Utah_kidney/202302_4_classes_tile_list/'
        tile_loc='/uu/sci.utah.edu/tolga_scratch/public_dataset/Utah_kidney/tiles_RGB_20X_size400_step200/'
    else:
    ...
    '''Baseline: r50 pretrained on pathological datasets(TCGA+TULIP)'''
    if my_modelname=='pathdino512':
        my_model = PathDinoClassifier(num_classes=NUM_CLASSES)
    else:
        my_model = ResNet50withFC(pretrained=True, progress=False, num_classes=NUM_CLASSES, key="SwAV", freeze=FREEZE_BACKBONE) # MoCoV2, SwAV, BT 
    #my_modelname = 'ResNet50Swav'
    ...
    if FREEZE_BACKBONE:
        ...
        if all_frozen:
            ...
            raise ValueError("Error: Not all feature extractor layers are frozen.")


    # my_model = models.resnet50(pretrained=True)#.cuda()#changable origin resnet18
    # my_modelname = 'ResNet50'#changable#####################
    ...
    #     my_model.fc.weight.requires_grad = True
    #     my_model.fc.bias.requires_grad = True
    ...
    if program_mode !='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
        ...
        if program_mode=='resume_best_training':# 'normal_training', 'resume_best_training', 'resume_latest_training', 'only_test'
            ...
            path2weights=save_models_folder+'/20240828_UtahPathdinoExp127_1_current_best.pt'
            my_model.load_state_dict(torch.load(path2weights))
            current_result_path=save_results_folder+'/20240828_UtahPathdinoExp127_1_current_best_for_resuming.txt'
            ...
            path2weights=save_models_folder+'/20240828_UtahPathdinoExp127_1_current_latest.pt'
            my_model.load_state_dict(torch.load(path2weights))
            current_result_path=save_results_folder+'/20240828_UtahPathdinoExp127_1_current_latest_for_resuming.txt'
        ...
        if FREEZE_BACKBONE:
            ...
            #optimizer = optim.RMSprop(filter(lambda p: p.requires_grad, my_model.parameters()), lr=initiall_lr, weight_decay=1e-8, momentum=0.9)
            # Set up the optimizer with only trainable parameters
            #my_model.fc.requires_grad = True
            trainable_params = list(filter(lambda p: p.requires_grad, my_model.parameters()))
            if len(trainable_params) == 0:
                raise ValueError("No trainable parameters found. Check the model's parameter settings.")
            optimizer = optim.RMSprop(filter(lambda p: p.requires_grad, my_model.parameters()), lr=initial_lr, weight_decay=1e-8, momentum=0.9)
            #optimizer = optim.Adam(filter(lambda p: p.requires_grad, my_model.parameters()), lr=initial_lr,weight_decay=0) 
        else:
            optimizer = optim.RMSprop(my_model.parameters(), lr=initial_lr, weight_decay=1e-8, momentum=0.9)
            #optimizer = optim.Adam(my_model.parameters(), lr=initial_lr,weight_decay=0) 
        ...
        for epoch in range(start_epoch, total_epoch+1):
            ...
            f = open(f'{save_results_folder}/20240828_UtahPathdinoExp127_1_val01_{my_modelname}.txt', 'a+')
            ...
            if best_acc<balanced_val_acc:
                ...
                get_best_model=1
                torch.save(my_model.state_dict(), save_models_folder+"/20240828_UtahPathdinoExp127_1_current_best.pt".format(my_modelname))
                ...
                now = datetime.now()
                current_time = now.strftime("%H:%M:%S")
                f = open(f'{save_results_folder}/20240828_UtahPathdinoExp127_1_test01_{my_modelname}.txt', 'a+')
            ...
            if epoch%save_latest_epoch_frequency==0:
                torch.save(my_model.state_dict(), save_models_folder+"/20240828_UtahPathdinoExp127_1_current_latest.pt".format(my_modelname))
                current_result_path=save_results_folder+'/20240828_UtahPathdinoExp127_1_current_latest_for_resuming.txt'
                current_result_file = open(current_result_path, 'w')#overwrite
                current_result_file.write(f'{epoch}\n')
                current_result_file.write(f'{balanced_val_acc}\n')
    ...
    if program_mode =='only_test': # 'normal_training', 'resume_best_training','resume_latest_training', 'only_test'
        path2test_weights=save_models_folder+"/20240828_UtahPathdinoExp127_1_current_best.pt"
        ...
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        f = open(f'{save_results_folder}/20240828_UtahPathdinoExp127_1_val01_load_model_{my_modelname}.txt', 'a+')
        ...
        now = datetime.now()
        current_time = now.strftime("%H:%M:%S")
        f = open(f'{save_results_folder}/20240828_UtahPathdinoExp127_1_test01_load_model_{my_modelname}.txt', 'a+')
        ...
        f.close()
    if get_best_model==1:
        torch.save(my_best_model.state_dict(), save_models_folder+"/20240828_UtahPathdinoExp127_1_val01_{}_{}_balanced_val_acc{}_balanced_test_acc{}.pt".format(my_modelname,best_epoch,best_acc,best_epoch_balanced_test_acc))
