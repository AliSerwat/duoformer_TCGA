...
def main():
    params = {'batch_size': 128, 'num_workers': 8, 'dataAug': True,
        'augType': 'random','is_distributed': False, 'normalize':'svhn'} # 128 for 2-layer scaleformer, 64 for 3-layer scaleformer, 16 for 4-layer scaleformer
    ...
    model = build_model(depth = depth, 
            ...
            freeze_backbone = freeze_backbone, backbone = backbone,pretrained=pretrained)
    # model = build_hybrid(num_classes=100,model_ver=model_ver) # R50ViTpretrained
    
    num_params, total_params = count_parameters(model)
    print("Trainable Parameter: \t%2.1fM" % num_params, "Total Parameter: \t%2.1fM" % total_params)

    train_dataloader,val_dataloader,test_dataloader = build_dataset('SVHN',params = params)
    ...
    for epoch in range(epochs):
        ...
        running_loss, running_accuracy = train(device,model, train_dataloader, criterion, optimizer, scheduler)
    ...
    ax.annotate(f'Max Test Acc: {best_test_acc:.2%}', xy=(best_test_epoch, best_test_acc),
                xytext=(best_test_epoch, best_test_acc + 5), textcoords='offset points',
                arrowprops=dict(arrowstyle="->", color='orange'))
    ax.annotate(f'Max Train Acc: {train_accs[best_test_epoch]:.2%}', xy=(best_test_epoch + 1, train_accs[best_test_epoch]),
                xytext=(0, -15), textcoords='offset points', ha='center', va='bottom', color='blue')

    ax.set_xlabel('Epochs')
    ax.set_ylabel('Accuracy')
    ax.set_title('Training and Testing Accuracy')
    ax.legend()
    plt.savefig('./save/cifar100' + model.name + 'Scales'+ str(model.num_layers) + 'lr'+ str(lr) + 'accuracy_plot.png')
    plt.show()
